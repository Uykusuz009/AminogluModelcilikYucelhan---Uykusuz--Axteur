...
setTimer(function ()
    ...
    -- Hız kontrolü ve otomatik spoiler kontrolü
    if vehicle and AnimationsTable[vehicle.model] then
        local speed = getVehicleSpeed(vehicle)  -- km/h'ye çevir
        local currentState = vehicle:getData("movparts_state")
        local newState = speed > 150
        
        if currentState ~= newState then
            setVehicleMovpartsState(vehicle, newState)
        end
    end
...
addCommandHandler("spoiler", function ()
    local vehicle = getDrivingVehicle()
    if vehicle then
...
-- Discord : https://discord.gg/DzgEcvy bu sistemde komut ile değilde arabanın hızı 100 ü geçince otomatik açılsın 100 den düşünce açılsın